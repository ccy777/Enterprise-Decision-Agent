"""Executable single-domain Skill for enterprise document knowledge QA."""
...
class EnterpriseKnowledgeQASkill:
    """Delegate knowledge requests to the existing native tool-calling runtime."""

    _definition = SkillDefinition(
        name="enterprise-knowledge-qa",
        version="1.0.0",
        description="Answer document knowledge questions through the existing cited Knowledge QA.",
        supported_route=RequestRoute.KNOWLEDGE,
        input_contract=("user_query", "RouterDecision(knowledge)"),
        allowed_tools=("run_knowledge_agent",),
        steps=("validate_route", "native_tool_calling", "validate_selected_tool", "map_result"),
        output_contract=("answer", "[E#] citations", "safe error_code"),
        failure_codes=(
            "skill_route_invalid",
    ...
    async def execute_with_context(
        ...
        *,
        ...
        if result.status is SkillStatus.COMPLETED and result.answer is not None:
            summary = context_runtime.verified_summary(
                "knowledge-answer-summary", result.answer, source_item_ids=(instruction.item_id,)
            )
    ...
    async def _execute(
        ...
        *,
        ...
        if decision.route is not RequestRoute.KNOWLEDGE:
