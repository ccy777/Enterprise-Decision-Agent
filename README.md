...
> **Knowledge + Data AI Agent Platform**
...
- **Agent Workflow 与 Prompt Engineering**：基于 LangGraph 编排 `Router -> Planner -> Skill -> Tool -> Reviewer` 多步骤工作流，通过 Prompt 与 Pydantic Structured Output 约束阶段输入输出。
- **Context Engineering 与会话记忆**：支持 Redis / In-Memory 会话状态、TTL、滚动摘要与上下文预算，按 tenant、user、session 隔离多轮任务。
- **Hybrid RAG 与检索评测**：`Dense + BM25 -> RRF -> Cross-Encoder -> Parent Expansion`，并通过 200-query Retrieval Benchmark v2 验证排序效果。
- **MCP Data Agent 与 Tool Calling**：通过 Native Tool Calling 选择并执行 Data Agent，由 Data Planner 生成结构化 SQL 计划，再调用 MCP Tools 完成 Schema 获取、MySQL 查询与结果分析。
- **Trace 与 Agent Evaluation**：记录 Routing、Planning、Skill、MCP、Evidence 与 Reviewer 的阶段状态、错误码和耗时，并建立离线回归与边界评测。
- **Knowledge / Data / Mixed 三类任务**：统一处理制度问答、经营数据分析以及知识与数据联合决策。
...
### 1. Agent Workflow 与 Prompt Engineering
...
### 3. MCP Data Agent 与 Tool Calling
...
| Deterministic boundary evaluation | 28 / 28 |
